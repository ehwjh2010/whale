...
Whale 内置 LSP 客户端，通过启动和管理语言服务器子进程，为代码智能功能提供支持。对外暴露 **9 个 LSP 工具**。LSP **默认关闭**，通过 `/lsp on` 启用（持久化到配置文件）；也可在 `whale.toml` 中设置 `[lsp] enabled = true`。
...
Whale 内置了 10 种常用语言的默认配置。启用 LSP 后（`/lsp on` 或 `[lsp] enabled = true`），只要对应语言的 LSP 二进制文件在 `PATH` 或已知安装目录中，即可自动发现，无需 lsp.json。

### /lsp 命令

| 用法 | 作用 |
|---|---|
| `/lsp` 或 `/lsp status` | 显示开关状态与各语言服务器可用性 |
| `/lsp on` | 启用 LSP（写入配置，重启后仍生效） |
| `/lsp off` | 关闭 LSP（写入配置；本会话立即关停已启动的服务器） |
| `/lsp sample` | 生成默认 lsp.json 服务器配置样例 |
